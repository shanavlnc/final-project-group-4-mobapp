  ...
  const updateApplicationStatus = async (id: string, status: 'approved' | 'rejected') => {
    try {
      ...
      if (status === 'approved') {
        ...
        await Promise.all([
          AsyncStorage.setItem('applications', JSON.stringify(finalApplications)),
          AsyncStorage.setItem('pets', JSON.stringify(updatedPets))
        ]);
        ...
        // Just update the application status if rejected
        await AsyncStorage.setItem('applications', JSON.stringify(updatedApplications));
        setState((prev: ApplicationState) => ({ ...prev, applications: updatedApplications }));
      }
    } catch (err) {
